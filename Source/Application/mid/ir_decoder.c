...
void ir_capture_timer_interrupts(nrf_timer_event_t event_type, void* p_context){
    ...
    switch (event_type)
    {
        case NRF_TIMER_EVENT_COMPARE0:
            ...
            NRF_LOG_INFO("Send IR");
//            for(uint8_t i = 0 ; i < ir_bit_index ; i++)
//            {
//            	NRF_LOG_INFO("%d. %d - %d", i, ir_signal_burst[i].duty_cycle_state, ir_signal_burst[i].duty_cycle_us);
//            }
            is_decoding = false;
            break;
        default:
...
void ir_in_gpiote_interrupt_handler(nrfx_gpiote_pin_t pin, nrf_gpiote_polarity_t action)
{
    ...
    else
    {
        ...
        ir_signal_burst[ir_bit_index].duty_cycle_us = pulse_width_us + 5; //+5 for considering CPU Cycles latency
        ir_bit_index++;
